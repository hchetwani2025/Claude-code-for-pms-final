...
- The routine "Review checklist runner" (Mondays 09:00 UTC, fresh session, push + email) is **paused** since 24 Sep; re-enable it from Routines to resume. Its one test run succeeded. It pulls the skill from branch `claude/magical-heisenberg-9xkpa5`, so skill changes must be pushed there. My time zone is not confirmed.
